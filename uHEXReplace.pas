...
function ReplaceGetOptions(var Options: TReplaceRec): Boolean;
begin
  with TfHEXReplace.Create(Application) do
  try
...
procedure TfHEXReplace.edFindChange(Sender: TObject);
begin
  Button1.Enabled := edFind.Text <> '';
  Button3.Enabled := edFind.Text <> '';
end;

procedure TfHEXReplace.FormCreate(Sender: TObject);
begin
  FReplaceAll := False;
end;

procedure TfHEXReplace.Button3Click(Sender: TObject);
begin
  FReplaceAll := True;
end;
